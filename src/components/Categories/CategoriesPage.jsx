...
class LocationsPage extends Component {
  ...
  updateToolBarState(event) {
    ...
    Object.keys(buttons).forEach(key => {
      if (key !== name) buttons[key].active = false;
    }); //reset active
  ...
  updateSortState(event) {
    ...
    if (categories.length > 1) {
      switch (this.state.sort.dir) {
        case "asc":
          filters["categories"] = utils.sortFunc(
            filters.categories || categories,
            prop,
            "desc"
          );
          this.setState({
            sort: { dir: "desc", icon: "fa fa-sort-up" },
            filters: filters
        ...
        default:
          ...
          this.setState({
            sort: { dir: "asc", icon: "fa fa-sort-down" },
            filters: filters
  ...
  render() {
    ...
    return (
      <div>
        <ToolBar
          buttons={this.state.toolbar}
          title="Categories"
          onClick={this.updateToolBarState}
          path="/category"
        />
        <CategoriesTable
          ...
          sort={sort}
          action={action}
          icon={this.getIcon(toolbar, action)}
        />
      </div>
    );
  }
